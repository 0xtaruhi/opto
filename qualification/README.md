...
| `processes-general-loop-state` | module-scope, multi-variable, and nested outer induction state, independently bounded runtime function arguments, distinct early-exit values, and post-loop runtime overwrite or partial update across all static loop forms | mapped synthesis against explicit finite control flow; CEC nightly |
