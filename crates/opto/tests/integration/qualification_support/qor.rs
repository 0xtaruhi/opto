...
pub(super) fn run(case: &Case, opto: &Path, yosys: &Path, output_root: &Path) -> ResultEntry {
    ...
    let commands = [
        ...
        format!(
            "tee -o {} stat -liberty {} -top {}",
            yosys_quote(output.join("yosys-stat.rpt")),
            yosys_quote(&liberty),
            case.spec.top
        ),
        format!(
    ...
    let (yosys_area, yosys_cells) = parse_yosys_area(&output.join("yosys-stat.rpt"));
...
mod tests {
    ...
    #[test]
    fn checked_in_qor_library_has_complete_timing_models() {
        ...
        assert_eq!(library.target_cells().len(), 11);
        let arcs = library
            .target_cells()
            .iter()
