...
use super::formal::{MappedProof, defined_output_designs, prove_mapped_equivalence};
...
fn load_audit() -> BTreeMap<String, AuditedExpectation> {
    ...
    assert_eq!(audit.format, 1, "Yosys audit format mismatch");
    let mut entries = BTreeMap::new();
    for (expectation, group) in [
        (Expectation::RejectSynthesis, audit.reject),
...
fn run_one(
    ...
    if matches!(
        ...
        tcl.push_str("set proof_index 0\n");
        tcl.push_str("foreach design [get_db designs] {\n");
        tcl.push_str("  set_db current_design $design\n");
        tcl.push_str("  if {[llength [get_ports *]] > 0} {\n");
        tcl.push_str("    synth\n");
        tcl.push_str("    puts \"OPTO_SYNTHESIZED\\t[get_db $design .name]\"\n");
        if test.expectation == Expectation::Pass {
            tcl.push_str("    if {[llength [get_ports -filter {direction == out} *]] > 0} {\n");
            writeln!(
                tcl,
                "      set proof_netlist [format \"%s/proof-%04d.v\" {} $proof_index]",
                tcl_word(&directory)
            )
            .expect("writing to a String cannot fail");
            tcl.push_str(
                "      write_hdl -hierarchy $proof_netlist\n      puts \"OPTO_CEC_TARGET\\t[get_db $design .name]\\t$proof_netlist\"\n      incr proof_index\n    }\n",
            );
        }
        tcl.push_str("  }\n}\n");
    }
    ...
    let reference_outputs = if test.expectation == Expectation::Pass {
        defined_output_designs(
            yosys,
    ...
    let actual_pass = process.status.success()
        && reference_outputs.is_some()
        && proved_designs == proof_targets.len();
    let matches_audit = match test.expectation {
        Expectation::Pass => actual_pass,
        Expectation::RejectFrontend | Expectation::RejectSynthesis | Expectation::KnownGap => {
            !actual_pass
        }
