...
CfgFragment ProcedureBuilder::effects(
...
CfgFragment ProcedureBuilder::sequence(
    CfgFragment first,
    CfgFragment second,
    OptoSlangSourceSpanView source) {
    if (first.empty()) {
...
CfgFragment ProcedureBuilder::guard(
...
CfgFragment ProcedureBuilder::conditional(
    const OptoSlangExpr* condition,
    ...
    branch(
        dispatch,
        condition,
        then_body.empty() ? *join : *then_body.entry,
        else_body.empty() ? *join : *else_body.entry,
        source);
    if (join) {
    ...
    return {dispatch, join ? std::vector<uint32_t>{*join} : std::vector<uint32_t>{}};
}

void ProcedureBuilder::jump(
    uint32_t from,
    uint32_t target,
    OptoSlangSourceSpanView source) {
...
uint32_t ProcedureBuilder::add_loop_region(OptoSlangLoopRegionData region) {
    if (loop_regions_.size() >= UINT32_MAX) {
        throw std::runtime_error("procedural loop-region arena exceeds 32-bit capacity");
    }
    ...
    if (region.header >= block_count || region.body >= block_count ||
        ...
        throw std::runtime_error("procedural loop region references an unknown block");
    }
    if (region.parent && *region.parent >= loop_regions_.size()) {
        throw std::runtime_error("procedural loop parent must be an earlier region");
    }
...
OptoSlangProcedureData ProcedureBuilder::finish(
    CfgFragment body,
...
uint32_t ProcedureBuilder::prune_unreachable(uint32_t entry) {
    ...
    while (!pending.empty()) {
        ...
        case CfgTerminatorKind::Pending:
            throw std::runtime_error("reachable procedural CFG block is unterminated");
...
OptoSlangProcedureData ProcedureBuilder::materialize(
    uint32_t entry,
    OptoSlangProcedureKind kind,
    std::vector<OptoSlangEventData> events,
    OptoSlangSourceSpanView source) {
    ...
    for (auto& block : blocks_) {
        ...
        output.jump_edge = {terminator.jump_edge.block, terminator.jump_edge.source};
        output.then_edge = {terminator.then_edge.block, terminator.then_edge.source};
        output.else_edge = {terminator.else_edge.block, terminator.else_edge.source};
        output.default_edge = {terminator.default_edge.block, terminator.default_edge.source};
        ...
        case CfgTerminatorKind::Switch:
            ...
            for (const auto& arm : terminator.arms) {
                output.arms.push_back(
                    {arm.pattern, {arm.edge.block, arm.edge.source}});
            }
            break;
        }
