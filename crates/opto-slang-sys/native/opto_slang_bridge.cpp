...
OptoSlangStatus fail(OptoSlangCompiler *compiler,
...
public:
  ...
  void report(const ReportedDiagnostic &reported) override {
    ...
    if (reported.location) {
      ...
      diagnostic.line = static_cast<uint32_t>(
          std::min<size_t>(sourceManager->getLineNumber(location), UINT32_MAX));
...
std::vector<OptoSlangDiagnostic>
collect_diagnostics(Driver &driver, const Diagnostics &diagnostics) {
...
bool contains_error(const std::vector<OptoSlangDiagnostic> &diagnostics) {
  return std::ranges::any_of(
      diagnostics, [](const OptoSlangDiagnostic &diagnostic) {
        return diagnostic.severity == OPTO_SLANG_DIAGNOSTIC_ERROR;
      });
}

OptoSlangStatus
diagnostic_view(const std::vector<OptoSlangDiagnostic> *diagnostics,
                size_t index, OptoSlangDiagnosticView *view) {
...
struct Verilog2005SyntaxValidator : SyntaxVisitor<Verilog2005SyntaxValidator> {
  ...
  void handle(const VariableDimensionSyntax &dimension) {
    if (!error && dimension.specifier &&
      ...
      const auto &range =
          dimension.specifier->as<RangeDimensionSpecifierSyntax>();
  ...
  void handle(const PortDeclarationSyntax &port) {
    if (!error &&
        std::ranges::any_of(port.declarators, [](const auto *declarator) {
  ...
  void handle(const ParameterPortListSyntax &parameters) {
    if (!error && std::ranges::any_of(
                      parameters.declarations, [](const auto *declaration) {
                        return declaration && !declaration->keyword;
                      })) {
      error = "parameter port declarations require the 'parameter' keyword in "
              "Verilog-2005";
    }
...
void validate_language_syntax(const SyntaxTree &tree,
...
std::string diagnostics_to_string(Driver &driver,
...
std::unique_ptr<Compilation> create_compilation(OptoSlangCompiler &compiler,
                                                Driver &driver) {
  ...
  driver.options.compilationFlags.at(CompilationFlags::IgnoreUnknownModules) =
      true;
    ...
    for (const auto &dependency : unit.dependencies) {
      ...
      if (!dependency.text) {
        throw std::runtime_error(
            "source dependency has no captured text for '" + dependency.path +
            "'");
      }
      auto [found, inserted] =
  ...
  for (const auto &[path, text] : dependency_text) {
    ...
    driver.sourceManager.setBufferKind(buffer.id,
  ...
  for (const auto &unit : compiler.units) {
    ...
    auto preprocessor =
        parse_options.getOrDefault<parsing::PreprocessorOptions>();
  ...
  auto parse = [&](size_t index) {
    try {
      ...
      syntax_trees[index] = SyntaxTree::fromBuffers(
          job.buffers, driver.sourceManager, job.options);
      if (!syntax_trees[index]) {
        throw std::runtime_error("slang failed to parse in-memory source unit");
      }
  ...
  auto compilation =
      std::make_unique<Compilation>(std::move(compilation_options));
...
void bind_procedural_bodies(
  ...
  for (const auto &symbol : scope.members()) {
    ...
    case SymbolKind::StatementBlock:
      bind_procedural_bodies(symbol.as<StatementBlockSymbol>(), visited_bodies);
      break;
    ...
    case SymbolKind::InstanceArray:
      for (auto *element : symbol.as<InstanceArraySymbol>().elements) {
        ...
        if (element->kind == SymbolKind::InstanceArray) {
          bind_procedural_bodies(element->as<InstanceArraySymbol>(),
                                 visited_bodies);
...
void opto_slang_compiler_free(OptoSlangCompiler *compiler) { delete compiler; }
...
opto_slang_compiler_add_source_dependency(OptoSlangCompiler *compiler,
  ...
  try {
    ...
    return fail(compiler,
                "unknown failure while adding a slang source dependency");
  }
}
...
OptoSlangStatus opto_slang_compiler_add_include_dir(OptoSlangCompiler *compiler,
  ...
  try {
    ...
    return fail(compiler,
                "unknown failure while adding a slang include directory");
  }
}
...
OptoSlangStatus opto_slang_compiler_compile(OptoSlangCompiler *compiler,
  ...
  *design = nullptr;
  ...
  try {
    ...
    opto_slang_collect_modules(*lowered, root->topInstances, std::move(driver),
...
OptoSlangStatus opto_slang_compiler_analyze(OptoSlangCompiler *compiler,
  ...
  *analysis = nullptr;
  ...
  try {
    ...
    result->definitions.erase(std::ranges::unique(result->definitions).begin(),
                              result->definitions.end());
    std::ranges::sort(result->packages);
    result->packages.erase(std::ranges::unique(result->packages).begin(),
                           result->packages.end());
    ...
    if (result->definitions.empty() && result->packages.empty()) {
      return fail(compiler,
                  "slang analysis produced no definitions or packages");
    }
...
opto_slang_compiler_diagnostic_view(const OptoSlangCompiler *compiler,
  ...
  return diagnostic_view(compiler ? &compiler->diagnostics : nullptr, index,
                         view);
}

void opto_slang_analysis_free(OptoSlangAnalysis *analysis) { delete analysis; }
...
const char *opto_slang_analysis_package_name(const OptoSlangAnalysis *analysis,
                                             size_t index) {
  return analysis && index < analysis->packages.size()
             ? analysis->packages[index].c_str()
             : nullptr;
}
...
opto_slang_analysis_diagnostic_view(const OptoSlangAnalysis *analysis,
                                    size_t index,
                                    OptoSlangDiagnosticView *view) {
  return diagnostic_view(analysis ? &analysis->diagnostics : nullptr, index,
                         view);
}
