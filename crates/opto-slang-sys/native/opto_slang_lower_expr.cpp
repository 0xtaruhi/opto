...
uint32_t scaled_dynamic_offset_width(uint32_t offset_width, uint32_t element_width) {
    if (element_width <= 1) {
        return offset_width;
    }
...
void apply_signal_slice(
    ModuleLoweringContext& design,
    OptoSlangExpr& signal,
    uint64_t relative_lsb,
    uint32_t width,
    const Expression& source) {
    if (signal.kind == OPTO_SLANG_EXPR_DYNAMIC_EXTRACT) {
        if (relative_lsb + width > signal.dynamic_extract_width || relative_lsb > UINT32_MAX) {
        ...
        if (relative_lsb != 0) {
            ...
            const auto offset_width =
                dynamic_offset_sum_width(signal.dynamic_extract_offset_width, relative_width);
            ...
            signal.dynamic_extract_offset = make_binary_expr(
                design,
                OPTO_SLANG_BINARY_ADD,
                offset,
                make_unsigned_constant_expr(design, relative_lsb_u32, offset_width, source),
                source);
    ...
    if (signal.kind != OPTO_SLANG_EXPR_SIGNAL) {
        throw std::runtime_error(
            "assignment slice base is neither a signal nor a "
            "dynamic signal selection");
    }
    const auto base_lsb =
        signal.signal_has_range ? std::min(signal.signal_msb, signal.signal_lsb) : 0;
    const auto lsb = static_cast<uint64_t>(base_lsb) + relative_lsb;
    const auto msb = lsb + width - 1;
    if (msb > UINT32_MAX) {
        throw std::runtime_error("selected signal slice exceeds 32-bit flattened range capacity");
    }
...
OptoSlangExpr* make_dynamic_extract(
    ModuleLoweringContext& design,
    ...
    if (value->kind == OPTO_SLANG_EXPR_DYNAMIC_EXTRACT) {
        if (width > value->dynamic_extract_width) {
            throw std::runtime_error("nested dynamic selection exceeds its containing element");
        }
        const auto combined_width =
            dynamic_offset_sum_width(value->dynamic_extract_offset_width, offset.width);
        offset.expression = make_binary_expr(
            design,
            OPTO_SLANG_BINARY_ADD,
            make_unsigned_cast_expr(design, value->dynamic_extract_offset, combined_width, source),
            make_unsigned_cast_expr(design, offset.expression, combined_width, source),
            source);
    ...
    if (value->kind == OPTO_SLANG_EXPR_SIGNAL && value->signal_has_range) {
        ...
        if (base_lsb != 0) {
            ...
            const auto combined_width = dynamic_offset_sum_width(offset.width, base_width);
            offset.expression = make_binary_expr(
                design,
                OPTO_SLANG_BINARY_ADD,
                make_unsigned_cast_expr(design, offset.expression, combined_width, source),
...
DynamicOffset make_dynamic_offset(
    ModuleLoweringContext& design,
    ...
    if (!selected_type.hasFixedRange()) {
        throw std::runtime_error(
            "dynamic select requires a fixed-size value at " + expression_location(design, source));
    }
    ...
    if (range.lower() < 0 || range.upper() < 0) {
        ...
        auto* selector = selector_expression.type->isSigned()
                             ? make_signed_cast_expr(
                                   design,
                                   lower_expr(design, selector_expression),
                                   offset_width,
                                   selector_expression)
                             : make_unsigned_cast_expr(
                                   design,
                                   lower_expr(design, selector_expression),
                                   offset_width,
                                   selector_expression);
        const auto origin = range.isDescending() ? range.lower() : range.upper();
        auto* origin_expr = make_signed_constant_expr(
            design, static_cast<int64_t>(origin), offset_width, selector_expression);
        const auto* offset = make_binary_expr(
            design,
            OPTO_SLANG_BINARY_SUB,
            range.isDescending() ? selector : origin_expr,
            range.isDescending() ? origin_expr : selector,
            selector_expression);
        const auto element_width = selected_element_width(selected_type);
        if (element_width > 1) {
            const auto scaled_width = scaled_dynamic_offset_width(offset_width, element_width);
            offset = make_binary_expr(
                design,
                OPTO_SLANG_BINARY_MUL,
                make_unsigned_cast_expr(design, offset, scaled_width, selector_expression),
                make_unsigned_constant_expr(
                    design, element_width, scaled_width, selector_expression),
                selector_expression);
            return DynamicOffset{offset, scaled_width};
        }
        return DynamicOffset{
            make_unsigned_cast_expr(design, offset, offset_width, selector_expression),
            offset_width,
        };
    }
    ...
    auto* selector = make_unsigned_cast_expr(
        design, lower_expr(design, selector_expression), offset_width, selector_expression);
    auto* bound_expr = make_unsigned_constant_expr(
        design,
        static_cast<uint32_t>(range.isDescending() ? range.lower() : range.upper()),
        offset_width,
        selector_expression);
    ...
    if (range.isDescending()) {
        offset =
            range.lower() == 0
                ? selector
                : make_binary_expr(
                      design, OPTO_SLANG_BINARY_SUB, selector, bound_expr, selector_expression);
    } else {
        offset = make_binary_expr(
            design, OPTO_SLANG_BINARY_SUB, bound_expr, selector, selector_expression);
    }
    const auto element_width = selected_element_width(selected_type);
    if (element_width > 1) {
        const auto scaled_width = scaled_dynamic_offset_width(offset_width, element_width);
        offset = make_binary_expr(
            ...
            OPTO_SLANG_BINARY_MUL,
            make_unsigned_cast_expr(design, offset, scaled_width, selector_expression),
            make_unsigned_constant_expr(design, element_width, scaled_width, selector_expression),
            selector_expression);
        return DynamicOffset{offset, scaled_width};
    }
...
OptoSlangExpr* make_dynamic_element_select(
    ModuleLoweringContext& design,
    const ElementSelectExpression& select,
    const OptoSlangExpr* value) {
    const auto& selected_type = *select.value().type;
    const auto offset = make_dynamic_offset(design, selected_type, select.selector(), select);
    return make_dynamic_extract(
        design, value, offset, selected_element_width(selected_type), select);
}

OptoSlangExpr* make_dynamic_indexed_part_select(
    ModuleLoweringContext& design,
    const RangeSelectExpression& select,
    const OptoSlangExpr* value,
    uint32_t selected_elements) {
    const auto& selected_type = *select.value().type;
    auto offset = make_dynamic_offset(design, selected_type, select.left(), select);
    const auto element_width = selected_element_width(selected_type);
    const auto selected_width = static_cast<uint64_t>(selected_elements) * element_width;
    if (selected_width == 0 || selected_width > UINT32_MAX) {
        throw std::runtime_error("dynamic indexed part-select width is out of range");
    }
    const auto range = selection_storage_range(selected_type);
    const bool base_is_high_end =
        (range.isDescending() && select.getSelectionKind() == RangeSelectionKind::IndexedDown) ||
        (!range.isDescending() && select.getSelectionKind() == RangeSelectionKind::IndexedUp);
    if (base_is_high_end && selected_width > element_width) {
        offset.expression = make_binary_expr(
            ...
            make_unsigned_constant_expr(
                design,
                static_cast<uint32_t>(selected_width - element_width),
                offset.width,
                select.left()),
            select.left());
    }
    return make_dynamic_extract(
        design, value, offset, static_cast<uint32_t>(selected_width), select);
}

OptoSlangExpr* lower_direct_child_port_reference(
    ModuleLoweringContext& design, const HierarchicalValueExpression& value) {
...
OptoSlangExpr* lower_signal_expr(ModuleLoweringContext& design, const Expression& expr) {
    switch (expr.kind) {
    ...
    case ExpressionKind::RangeSelect: {
        ...
        if (!left) {
            if (!right || select.getSelectionKind() == RangeSelectionKind::Simple) {
                throw std::runtime_error("dynamic part-select requires a constant indexed width");
            }
        ...
        if (right_index > static_cast<uint64_t>(INT32_MAX) ||
            *left > static_cast<uint32_t>(INT32_MAX)) {
            throw std::runtime_error("range select index exceeds 32-bit signed capacity");
        }
        const auto& selected_type = *select.value().type;
        if (!selected_type.hasFixedRange()) {
        ...
        if (!declared.containsPoint(left_index) || !declared.containsPoint(right_index_i32)) {
        ...
        const auto relative_lsb =
            static_cast<uint64_t>(std::min(left_offset, right_offset)) * element_width;
        const auto distance =
            std::abs(static_cast<int64_t>(left_offset) - static_cast<int64_t>(right_offset));
        ...
        apply_signal_slice(design, *base, relative_lsb, static_cast<uint32_t>(width), expr);
        return base;
    }
...
bool constant_element_select_is_out_of_range(
    ...
    return !selection_storage_range(selected_type).containsPoint(static_cast<int32_t>(*index));
}

OptoSlangExpr* make_unknown_expression(
    ModuleLoweringContext& design, const Expression& expression) {
    OptoSlangExpr lowered;
...
void collect_lvalue_leaves(const Expression& expression, std::vector<LvalueLeaf>& leaves) {
    if (expression.kind == ExpressionKind::Concatenation) {
        for (auto* operand : expression.as<ConcatenationExpression>().operands()) {
            if (!operand) {
                throw std::runtime_error("lvalue concatenation contains an empty operand");
            }
...
lower_continuous_assignment(ModuleLoweringContext& design, const AssignmentExpression& assignment) {
    ...
    for (const auto& leaf : leaves) {
        ...
        auto* slice =
            apply_rvalue_slice(design, rhs, total_width - consumed, leaf.width, assignment.right());
        lowered.push_back(
            OptoSlangAssignData{
    ...
    if (consumed != total_width) {
        throw std::runtime_error("lvalue concatenation width does not match its assignment type");
    }
    return lowered;
}

OptoSlangExpr* lower_select_expr(ModuleLoweringContext& design, const Expression& expr) {
    if (expr.kind == ExpressionKind::ElementSelect) {
        ...
        auto index = integer_literal_u32(design, select.selector());
        if (!index) {
            return make_dynamic_element_select(design, select, lower_expr(design, select.value()));
        }
        const auto& selected_type = *select.value().type;
        if (!selected_type.hasFixedRange() || *index > static_cast<uint32_t>(INT32_MAX)) {
            throw std::runtime_error("element select requires a fixed 32-bit index range");
        }
        const auto range = selection_storage_range(selected_type);
        ...
        return apply_rvalue_slice(
            ...
            static_cast<uint64_t>(translated) * width,
            width,
            expr);
    }
...
OptoSlangExpr* lower_constant_value(
    ModuleLoweringContext& design,
    const Type& type,
    ConstantValue value,
    const Expression& source,
    ...
    if (!value.isInteger()) {
        ...
        value = Bitstream::convertToBitVector(std::move(value), source.sourceRange, context);
    }
    if (!value || !value.isInteger()) {
        throw std::runtime_error(
            "constant '" + copy_string(description) + "' could not be converted to a bit vector");
    }
...
OptoSlangExpr* lower_streaming_concatenation(
    ModuleLoweringContext& design, const StreamingConcatenationExpression& streaming) {
    if (!streaming.isFixedSize() || streaming.getBitstreamWidth() == 0 ||
        streaming.getBitstreamWidth() > UINT32_MAX) {
        throw std::runtime_error("streaming concatenation requires a fixed nonzero 32-bit width");
    }
    ...
    for (const auto& stream : streaming.streams()) {
        if (stream.withExpr) {
            throw std::runtime_error(
                "streaming concatenation with-clauses are not "
    ...
    for (uint32_t offset = 0; offset < total_width;) {
        const auto width =
            static_cast<uint32_t>(std::min<uint64_t>(slice_size, total_width - offset));
...
OptoSlangExpr* lower_inside_item_match(
    ModuleLoweringContext& design, const OptoSlangExpr* value, const Expression& item) {
    if (item.kind == ExpressionKind::ValueRange) {
        const auto& range = item.as<ValueRangeExpression>();
        if (range.rangeKind != ValueRangeKind::Simple || range.left().type->isUnbounded() ||
            range.right().type->isUnbounded()) {
            throw std::runtime_error(
                "only bounded simple inside ranges are supported for synthesis");
        }
        auto* at_least = make_binary_expr(
            design, OPTO_SLANG_BINARY_GE, value, lower_expr(design, range.left()), item);
        auto* at_most = make_binary_expr(
            design, OPTO_SLANG_BINARY_LE, value, lower_expr(design, range.right()), item);
        return make_binary_expr(design, OPTO_SLANG_BINARY_LOGICAL_AND, at_least, at_most, item);
    }

    auto* matched_value = lower_expr(design, item);
    if (matched_value->kind != OPTO_SLANG_EXPR_CONSTANT || !matched_value->constant_has_width ||
        matched_value->constant_bits.find_first_of("xXzZ") == std::string::npos) {
        return make_binary_expr(design, OPTO_SLANG_BINARY_EQ, value, matched_value, item);
    }
    ...
    auto* masked = make_binary_expr(design, OPTO_SLANG_BINARY_BIT_AND, value, mask_value, item);
...
OptoSlangExpr* lower_expr(ModuleLoweringContext& design, const Expression& expr) {
    if (expr.kind == ExpressionKind::MemberAccess) {
        ...
        if (constant) {
            return lower_constant_value(
                design, *expr.type, std::move(constant), expr, access.member, access.member.name);
        }
    }
    if (expr.kind == ExpressionKind::NamedValue) {
        ...
        if (auto found = design.procedural_constants.find(&symbol);
            ...
            if (!found->second.isInteger()) {
                throw std::runtime_error("procedural loop variable has a non-integral value");
            }
    ...
    if (ValueExpressionBase::isKind(expr.kind)) {
        ...
        if (constant) {
            return lower_constant_value(
                design, symbol.getType(), *constant, expr, symbol, symbol.name);
        }
    }
    if (expr.kind != ExpressionKind::IntegerLiteral) {
        if (auto* constant = expr.getConstant(); constant && constant->isInteger()) {
            ...
            lowered.kind = OPTO_SLANG_EXPR_CONSTANT;
            lowered.constant_has_width = true;
            lowered.constant_width = checked_width(value.getBitWidth(), "constant expression");
            lowered.constant_bits = exact_binary_string(value);
            return make_expr(design, std::move(lowered), expr);
        }
    }
    if (expr.kind == ExpressionKind::Call && !expr.as<CallExpression>().isSystemCall()) {
        ...
        if (constant && constant.isInteger()) {
            ...
            lowered.kind = OPTO_SLANG_EXPR_CONSTANT;
            lowered.constant_has_width = true;
            lowered.constant_width = checked_width(value.getBitWidth(), "constant function call");
            lowered.constant_bits = exact_binary_string(value);
            return make_expr(design, std::move(lowered), expr);
        }
    }
    ...
    case ExpressionKind::Invalid: {
        ...
        if (!child) {
            throw std::runtime_error("invalid expression at " + expression_location(design, expr));
        }
    ...
    case ExpressionKind::IntegerLiteral: {
        ...
        lowered.kind = OPTO_SLANG_EXPR_CONSTANT;
        lowered.constant_has_width = true;
        lowered.constant_width = checked_width(value.getBitWidth(), "integer literal");
        lowered.constant_bits = exact_binary_string(value);
        return make_expr(design, std::move(lowered), expr);
    }
    ...
    case ExpressionKind::UnaryOp: {
        ...
        if (unary.op == UnaryOperator::BitwiseNand || unary.op == UnaryOperator::BitwiseNor ||
            unary.op == UnaryOperator::BitwiseXnor) {
            const auto reduction =
                unary.op == UnaryOperator::BitwiseNand  ? OPTO_SLANG_UNARY_REDUCTION_AND
                : unary.op == UnaryOperator::BitwiseNor ? OPTO_SLANG_UNARY_REDUCTION_OR
                                                        : OPTO_SLANG_UNARY_REDUCTION_XOR;
            auto* reduced =
                make_unary_expr(design, reduction, lower_expr(design, unary.operand()), expr);
    ...
    case ExpressionKind::BinaryOp: {
        ...
        if (binary.op == BinaryOperator::BinaryXnor) {
            auto* xored = make_binary_expr(
                design,
                OPTO_SLANG_BINARY_BIT_XOR,
                lower_expr(design, binary.left()),
                lower_expr(design, binary.right()),
                expr);
    ...
    case ExpressionKind::Concatenation: {
        ...
        for (auto* part : concat.operands()) {
            ...
            if (part->kind == ExpressionKind::Replication) {
                ...
                if (!count) {
                    throw std::runtime_error("replication expression requires a constant count");
                }
                if (*count == 0) {
                    continue;
                }
    ...
    case ExpressionKind::Replication: {
        ...
        if (part->kind == OPTO_SLANG_EXPR_CONCAT) {
            ...
            for (uint32_t index = 0; index < *count; ++index) {
                lowered.concat_parts.insert(
                    lowered.concat_parts.end(),
    ...
    case ExpressionKind::Inside: {
        ...
        for (auto* item : inside.rangeList()) {
            ...
            condition = condition
                            ? make_binary_expr(
                                  design, OPTO_SLANG_BINARY_LOGICAL_OR, condition, matched, *item)
                            : matched;
        }
        if (!condition) {
    ...
    case ExpressionKind::Conversion: {
        ...
        const auto streaming_width =
            operand.kind == ExpressionKind::Streaming
                ? std::optional<uint64_t>(
                      operand.as<StreamingConcatenationExpression>().getBitstreamWidth())
                : std::nullopt;
        if (!expr.type->isIntegral() || (!streaming_width && (!operand.type->isBitstreamType() ||
                                                              !operand.type->isFixedSize()))) {
            throw std::runtime_error(
                "only fixed bitstream conversion expressions "
        ...
        const bool sign_extend = conversion.conversionKind == ConversionKind::Propagated
                                     ? expr.type->isSigned()
    ...
    case ExpressionKind::Call: {
        ...
        if (constant && constant.isInteger()) {
            ...
            lowered.kind = OPTO_SLANG_EXPR_CONSTANT;
            lowered.constant_has_width = true;
            lowered.constant_width = checked_width(value.getBitWidth(), "constant system call");
            lowered.constant_bits = exact_binary_string(value);
            return make_expr(design, std::move(lowered), expr);
        }
