...
typedef enum OptoSlangEdge {
  OPTO_SLANG_EDGE_POS = 0,
  OPTO_SLANG_EDGE_NEG = 1
} OptoSlangEdge;
...
OptoSlangStatus
opto_slang_compiler_begin_source_unit(OptoSlangCompiler *compiler);
OptoSlangStatus opto_slang_compiler_add_source_file(OptoSlangCompiler *compiler,
                                                    const char *path,
                                                    const char *text);
OptoSlangStatus opto_slang_compiler_add_source_path(OptoSlangCompiler *compiler,
                                                    const char *path);
OptoSlangStatus
opto_slang_compiler_add_source_dependency(OptoSlangCompiler *compiler,
                                          const char *path, const char *text);
OptoSlangStatus opto_slang_compiler_add_include_dir(OptoSlangCompiler *compiler,
                                                    const char *path);
OptoSlangStatus opto_slang_compiler_add_define(OptoSlangCompiler *compiler,
                                               const char *name,
                                               const char *value);
OptoSlangStatus opto_slang_compiler_set_top(OptoSlangCompiler *compiler,
                                            const char *top);
OptoSlangStatus opto_slang_compiler_set_language(OptoSlangCompiler *compiler,
                                                 int language);
OptoSlangStatus opto_slang_compiler_set_max_threads(OptoSlangCompiler *compiler,
                                                    uint32_t max_threads);
OptoSlangStatus opto_slang_compiler_compile(OptoSlangCompiler *compiler,
                                            OptoSlangSnapshot **design);
OptoSlangStatus opto_slang_compiler_analyze(OptoSlangCompiler *compiler,
...
OptoSlangStatus
opto_slang_compiler_diagnostic_view(const OptoSlangCompiler *compiler,
                                    size_t index,
                                    OptoSlangDiagnosticView *view);

void opto_slang_analysis_free(OptoSlangAnalysis *analysis);
OptoSlangStatus opto_slang_analysis_view(const OptoSlangAnalysis *analysis,
                                         OptoSlangAnalysisView *view);
const char *
opto_slang_analysis_definition_name(const OptoSlangAnalysis *analysis,
                                    size_t index);
const char *opto_slang_analysis_package_name(const OptoSlangAnalysis *analysis,
                                             size_t index);
OptoSlangStatus
opto_slang_analysis_dependency_view(const OptoSlangAnalysis *analysis,
                                    size_t index,
                                    OptoSlangSourceFileView *view);
size_t opto_slang_analysis_diagnostic_count(const OptoSlangAnalysis *analysis);
OptoSlangStatus
opto_slang_analysis_diagnostic_view(const OptoSlangAnalysis *analysis,
                                    size_t index,
                                    OptoSlangDiagnosticView *view);

void opto_slang_snapshot_free(OptoSlangSnapshot *design);
OptoSlangStatus opto_slang_snapshot_view(const OptoSlangSnapshot *design,
                                         OptoSlangSnapshotView *view);
size_t opto_slang_snapshot_diagnostic_count(const OptoSlangSnapshot *design);
OptoSlangStatus
opto_slang_snapshot_diagnostic_view(const OptoSlangSnapshot *design,
                                    size_t index,
                                    OptoSlangDiagnosticView *view);
OptoSlangStatus opto_slang_module_view(const OptoSlangSnapshot *design,
                                       size_t module_index,
                                       OptoSlangModuleView *view);
OptoSlangStatus opto_slang_module_info(const OptoSlangSnapshot *design,
                                       size_t module_index,
                                       OptoSlangModuleInfoView *view);
OptoSlangStatus opto_slang_module_materialize(OptoSlangSnapshot *design,
                                              size_t module_index);
OptoSlangStatus
opto_slang_module_materialize_failure(const OptoSlangSnapshot *design,
                                      size_t module_index,
                                      OptoSlangLoweringFailureView *view);
void opto_slang_module_release(OptoSlangSnapshot *design, size_t module_index);

OptoSlangStatus
opto_slang_module_attribute_view(const OptoSlangSnapshot *design,
                                 size_t module_index, size_t attribute_index,
                                 OptoSlangAttributeView *view);

OptoSlangStatus opto_slang_port_view(const OptoSlangSnapshot *design,
                                     size_t module_index, size_t port_index,
                                     OptoSlangPortView *view);
OptoSlangStatus opto_slang_port_attribute_view(const OptoSlangPortData *port,
                                               size_t attribute_index,
                                               OptoSlangAttributeView *view);
OptoSlangStatus opto_slang_net_view(const OptoSlangSnapshot *design,
                                    size_t module_index, size_t net_index,
                                    OptoSlangNetView *view);
OptoSlangStatus opto_slang_net_attribute_view(const OptoSlangNetData *net,
                                              size_t attribute_index,
                                              OptoSlangAttributeView *view);
OptoSlangStatus opto_slang_instance_view(const OptoSlangSnapshot *design,
                                         size_t module_index,
                                         size_t instance_index,
                                         OptoSlangInstanceView *view);
OptoSlangStatus
opto_slang_instance_attribute_view(const OptoSlangInstanceData *instance,
                                   size_t attribute_index,
                                   OptoSlangAttributeView *view);
OptoSlangStatus
opto_slang_connection_view(const OptoSlangInstanceData *instance,
                           size_t connection_index,
                           OptoSlangConnectionView *view);
OptoSlangStatus opto_slang_assign_view(const OptoSlangSnapshot *design,
                                       size_t module_index, size_t assign_index,
                                       OptoSlangAssignView *view);
OptoSlangStatus opto_slang_procedure_view(const OptoSlangSnapshot *design,
                                          size_t module_index,
                                          size_t procedure_index,
                                          OptoSlangProcedureView *view);
OptoSlangStatus opto_slang_event_view(const OptoSlangProcedureData *procedure,
                                      size_t event_index,
                                      OptoSlangEventView *view);
OptoSlangStatus opto_slang_block_view(const OptoSlangProcedureData *procedure,
                                      size_t block_index,
                                      OptoSlangBlockView *view);
OptoSlangStatus
opto_slang_loop_region_view(const OptoSlangProcedureData *procedure,
                            size_t region_index, OptoSlangLoopRegionView *view);
OptoSlangStatus opto_slang_effect_view(const OptoSlangProcedureData *procedure,
                                       size_t block_index, size_t effect_index,
                                       OptoSlangEffectView *view);
OptoSlangStatus
opto_slang_terminator_view(const OptoSlangProcedureData *procedure,
                           size_t block_index, OptoSlangTerminatorView *view);
OptoSlangStatus
opto_slang_switch_arm_view(const OptoSlangProcedureData *procedure,
                           size_t block_index, size_t arm_index,
                           OptoSlangSwitchArmView *view);
OptoSlangStatus opto_slang_type_layout_view(const OptoSlangTypeLayout *layout,
                                            OptoSlangTypeLayoutView *view);
OptoSlangStatus opto_slang_type_field_view(const OptoSlangTypeLayout *layout,
                                           size_t field_index,
                                           OptoSlangTypeFieldView *view);
OptoSlangStatus opto_slang_expr_view(const OptoSlangExpr *expression,
                                     OptoSlangExprView *view);
