...
public:
  LoweringFailure(OptoSlangLoweringFailureCategory category, uint16_t code,
                  slang::SourceLocation location, std::string message)
...
public:
  ...
  ScopedValue(T &target, T replacement)
      : target(target),
        previous(std::exchange(target, std::move(replacement))) {}
  ScopedValue(const ScopedValue &) = delete;
  ScopedValue &operator=(const ScopedValue &) = delete;
  ~ScopedValue() { target = std::move(previous); }
...
struct ModuleLoweringContext {
  explicit ModuleLoweringContext(
      OptoSlangModulePayload &module, const InstanceBodySymbol &body,
      const std::unordered_map<const InstanceBodySymbol *, std::string>
          &body_names,
      const SourceManager *source_manager)
      : module(module), body(body), body_names(body_names),
  ...
  std::unordered_map<const Type *, const OptoSlangTypeLayout *>
      type_layout_by_type;
...
template <typename Value> class ScopedSymbolMapBindings {
public:
  explicit ScopedSymbolMapBindings(
      std::unordered_map<const ValueSymbol *, Value> &bindings)
      : bindings_(bindings) {}
...
OptoSlangExpr *lower_function_call(ModuleLoweringContext &,
                                   const CallExpression &);
OptoSlangExpr *lower_assignment_expression(ModuleLoweringContext &,
                                           const AssignmentExpression &);
OptoSlangExpr *lower_update_expression(ModuleLoweringContext &,
                                       const UnaryExpression &);
OptoSlangExpr *lower_conditional_expression(ModuleLoweringContext &,
                                            const ConditionalExpression &);
OptoSlangExpr *lower_short_circuit_expression(ModuleLoweringContext &,
...
lower_continuous_assignment(ModuleLoweringContext &,
                            const AssignmentExpression &);
OptoSlangExpr *lower_inside_item_match(ModuleLoweringContext &,
                                       const OptoSlangExpr *,
                                       const Expression &);
std::optional<bool> constant_boolean_value(ModuleLoweringContext &,
                                           const Expression &);
OptoSlangExpr *lower_expr(ModuleLoweringContext &, const Expression &);
OptoSlangProcedureData lower_procedure(ModuleLoweringContext &,
                                       const InstanceBodySymbol &,
                                       const ProceduralBlockSymbol &);
OptoSlangProcedureData make_guarded_procedure(std::vector<GuardedEffectData>,
                                              OptoSlangProcedureKind,
                                              std::vector<OptoSlangEventData>,
                                              OptoSlangSourceSpanView);
void validate_initial_process(ModuleLoweringContext &,
                              const InstanceBodySymbol &,
                              const ProceduralBlockSymbol &);
} // namespace opto::slang_lower
