...
[[noreturn]] void reject_external_port_projection(SourceLocation location,
                                                  std::string message) {
  throw LoweringFailure(OPTO_SLANG_LOWERING_INVALID_PROJECTION, 1, location,
                        std::move(message));
}

const ValueSymbol &
external_port_internal_value(const PortSymbol &port, SourceLocation location,
  ...
  if (!port.internalSymbol || !ValueSymbol::isKind(port.internalSymbol->kind)) {
    reject_external_port_projection(
        location, "external port '" + std::string(external_name) +
                      "' does not map to an integral internal value");
  }
  return port.internalSymbol->as<ValueSymbol>();
}

OptoSlangExpr *make_external_port_slice(ModuleLoweringContext &design,
                                        const std::string &name,
                                        uint32_t total_width, uint32_t lsb,
                                        uint32_t width) {
...
OptoSlangExpr *lower_external_port_part(ModuleLoweringContext &design,
                                        const ExternalPortPart &part,
                                        bool lvalue) {
  if (!part.port) {
    throw std::logic_error(
        "external port projection contains a null component");
  }
  if (auto *expression = part.port->getInternalExpr()) {
    return lvalue ? lower_signal_expr(design, *expression)
                  : lower_expr(design, *expression);
  }
...
void validate_external_port_parts(const ExternalPortProjection &projection,
  ...
  for (size_t index = 0; index < parts.size(); ++index) {
    ...
    if (std::ranges::any_of(ranges, [lsb, msb](const auto &range) {
      ...
      reject_external_port_projection(
          projection.location, "external port '" + projection.name +
  ...
  for (const auto &projection : projections) {
    ...
    for (const auto &part : projection.parts) {
      ...
      if (total_width > UINT32_MAX) {
        reject_external_port_projection(projection.location,
                                        "external port '" + projection.name +
                                            "' width exceeds 32-bit capacity");
      }
    }
    if (projection.parts.empty() || total_width != projection.width) {
      reject_external_port_projection(
          projection.location, "external port '" + projection.name +
                                   "' has an internal projection width that "
                                   "does not match its declared width");
    }
    ...
    if (projection.direction == ArgumentDirection::In) {
      ...
      for (size_t index = 0; index < projection.parts.size(); ++index) {
        ...
        consumed += width;
        design.module.assigns.push_back(OptoSlangAssignData{
            lowered_parts[index],
            make_external_port_slice(
                design, projection.name, projection.width,
                static_cast<uint32_t>(projection.width - consumed), width),
        });
      }
      continue;
    }
...
bool is_udp_edge_symbol(char symbol) {
  return symbol == '*' || symbol == 'r' || symbol == 'f' || symbol == 'p' ||
         symbol == 'n';
}

std::vector<UdpInputField>
parse_udp_input_fields(const PrimitiveInstanceSymbol &instance,
  ...
  for (size_t offset = 0; offset < row.inputs.size();) {
    ...
    if (symbol != '(') {
      fields.push_back(
          {row.inputs.substr(offset, 1), is_udp_edge_symbol(symbol)});
      ++offset;
      continue;
    }
    if (offset + 3 >= row.inputs.size() || row.inputs[offset + 3] != ')') {
      throw std::runtime_error("UDP '" +
...
UdpBinaryEdges lower_udp_binary_edges(const PrimitiveInstanceSymbol &instance,
                                      const UdpInputField &field) {
  if (field.symbols.size() == 1) {
    ...
    default:
      throw std::runtime_error("UDP '" +
                               copy_string(instance.primitiveType.name) +
                               "' has unsupported transition symbol '" +
                               copy_string(field.symbols) + "'");
    }
  }
...
UdpRowEdge inspect_udp_row_edge(const PrimitiveInstanceSymbol &instance,
                                const PrimitiveSymbol::TableEntry &row,
                                size_t input_count) {
  UdpRowEdge result;
  bool saw_edge = false;
  ...
  if (!saw_edge) {
    throw std::runtime_error("edge-sensitive UDP '" +
                             copy_string(instance.primitiveType.name) +
...
UdpAsyncControl
inspect_udp_async_control(const PrimitiveInstanceSymbol &instance,
                          const PrimitiveSymbol::TableEntry &row,
                          size_t input_count) {
...
void append_udp_binary_match(ModuleLoweringContext &design,
                             UdpBinaryPredicate &predicate, char symbol,
  ...
  case 'x':
    ...
    return;
  default:
    throw std::runtime_error("UDP '" + copy_string(primitive) +
                             "' has unsupported level symbol '" +
                             std::string(1, symbol) + "'");
  }
  predicate.expression =
      predicate.expression
          ? make_binary_expr(design, OPTO_SLANG_BINARY_BIT_AND,
                             predicate.expression, term, source)
          : term;
}

UdpBinaryPredicate lower_udp_level_predicate(
    ModuleLoweringContext &design, const PrimitiveInstanceSymbol &instance,
    std::span<const Expression *const> ports,
    const PrimitiveSymbol::TableEntry &row, const Expression *current_state) {
  UdpBinaryPredicate predicate;
  ...
  for (const auto &field : parse_udp_input_fields(instance, row)) {
    ...
    append_udp_binary_match(design, predicate, field.symbols[0],
                            lower_expr(design, input), input,
  ...
  if (current_state) {
    if (!row.state) {
      throw std::runtime_error("sequential UDP '" +
                               copy_string(instance.primitiveType.name) +
                               "' table row has no current state");
    }
    append_udp_binary_match(design, predicate, row.state,
                            lower_expr(design, *current_state), *current_state,
                            instance.primitiveType.name);
  } else if (row.state) {
    throw std::runtime_error("combinational UDP '" +
                             copy_string(instance.primitiveType.name) +
...
UdpEdgePredicate lower_udp_edge_predicate(
  ...
  for (const auto &field : parse_udp_input_fields(instance, row)) {
    ...
    if (!field.is_edge) {
      ...
      append_udp_binary_match(design, lowered.predicate, field.symbols[0],
                              value, input, instance.primitiveType.name);
    } else {
      if (!lowered.edges.pos && !lowered.edges.neg) {
        lowered.predicate.reachable = false;
      } else if (lowered.edges.pos != lowered.edges.neg &&
                 (!implicit_event_port ||
                  lowered.event_port != *implicit_event_port)) {
        append_udp_binary_match(design, lowered.predicate,
                                lowered.edges.pos ? '1' : '0', value, input,
                                instance.primitiveType.name);
      }
    }
    ++input_index;
  ...
  append_udp_binary_match(design, lowered.predicate, row.state,
                          lower_expr(design, current_state), current_state,
                          instance.primitiveType.name);
  return lowered;
}

OptoSlangExpr *make_udp_logic_value(ModuleLoweringContext &design, char value,
                                    const Expression &source) {
  if (value == '0' || value == '1') {
    return make_unsigned_constant_expr(design, value == '1' ? 1 : 0, 1, source);
  }
  if (value != 'x') {
    throw std::runtime_error("UDP table has unsupported next-state symbol '" +
                             std::string(1, value) + "'");
  }
...
OptoSlangAttributeData lower_attribute(ModuleLoweringContext &design,
  ...
  const auto *attribute_syntax =
      syntax && syntax->kind == SyntaxKind::AttributeSpec
          ...
          : nullptr;
  const auto *expression_syntax = attribute_syntax && attribute_syntax->value
                                      ? attribute_syntax->value->expr.get()
                                      : nullptr;
  if (expression_syntax &&
      expression_syntax->kind == SyntaxKind::StringLiteralExpression) {
    result.kind = OPTO_SLANG_ATTRIBUTE_STRING;
    result.value =
        expression_syntax->as<LiteralExpressionSyntax>().literal.valueText();
...
void append_symbol_attributes(ModuleLoweringContext &design,
                              const Symbol &symbol,
...
std::vector<OptoSlangAttributeData>
lower_symbol_attributes(ModuleLoweringContext &design, const Symbol &symbol) {
  std::vector<OptoSlangAttributeData> lowered;
  std::unordered_set<const AttributeSymbol *> seen;
...
std::vector<OptoSlangAttributeData>
lower_port_attributes(ModuleLoweringContext &design, const PortSymbol &port) {
  std::vector<OptoSlangAttributeData> lowered;
  std::unordered_set<const AttributeSymbol *> seen;
...
std::vector<OptoSlangAttributeData>
lower_multi_port_attributes(ModuleLoweringContext &design,
                            const MultiPortSymbol &port) {
  std::vector<OptoSlangAttributeData> lowered;
  std::unordered_set<const AttributeSymbol *> seen;
  append_symbol_attributes(design, port, seen, lowered);
...
bool unpacked_element_is_signed(const Type &source_type) {
  ...
  while (type->kind == SymbolKind::FixedSizeUnpackedArrayType) {
    type =
        &type->as<FixedSizeUnpackedArrayType>().elementType.getCanonicalType();
  }
  return type->isSigned();
}

void add_value_as_net(ModuleLoweringContext &design,
                      OptoSlangModulePayload &module,
                      std::unordered_set<std::string> &existing,
                      const ValueSymbol &symbol, std::string name,
  ...
  OptoSlangNetData net{
      ...
      false,
      symbol.kind == SymbolKind::Net ? net_resolution(symbol.as<NetSymbol>())
                                     : OPTO_SLANG_NET_SINGLE_DRIVER,
...
const Expression &
instance_connection_expression(const PortConnection &connection) {
...
void lower_primitive_instance(ModuleLoweringContext &design,
                              OptoSlangModulePayload &module,
                              const PrimitiveInstanceSymbol &instance) {
  if (instance.getDelay()) {
    throw std::runtime_error("delayed primitive instance '" +
                             copy_string(instance.name) +
                             "' is not synthesizable");
  }
  const auto [drive0, drive1] = instance.getDriveStrength();
  if (drive0 || drive1) {
    throw std::runtime_error("drive strength on primitive instance '" +
                             copy_string(instance.name) +
  ...
  auto require_count = [&](size_t minimum,
                           std::optional<size_t> exact = std::nullopt) {
    if ((exact && ports.size() != *exact) ||
        (!exact && ports.size() < minimum)) {
      throw std::runtime_error("primitive '" + primitive + "' instance '" +
                               copy_string(instance.name) + "' has " +
  ...
  auto append_assign = [&](size_t output_index, const OptoSlangExpr *rhs) {
    ...
    module.assigns.push_back(OptoSlangAssignData{
  ...
  if (instance.primitiveType.primitiveKind == PrimitiveSymbol::UserDefined) {
    ...
    if (udp.ports.size() - 1 > UDP_INPUT_LIMIT) {
      throw std::runtime_error("UDP '" + primitive +
                               "' exceeds the deterministic input limit of " +
                               std::to_string(UDP_INPUT_LIMIT));
    }
    if (udp.table.size() > UDP_TABLE_ROW_LIMIT) {
      throw std::runtime_error(
          "UDP '" + primitive +
          "' exceeds the deterministic table-row limit of " +
    ...
    if (udp.isSequential) {
      ...
      if (udp.isEdgeSensitive) {
        ...
        for (const auto &row : udp.table) {
          ...
          if (!row.isEdgeSensitive) {
            ...
            if (control.reachable) {
              async_controls.emplace_back(control.event_port,
                                          control.active_high
                                              ? OPTO_SLANG_EDGE_POS
                                              : OPTO_SLANG_EDGE_NEG);
            }
        ...
        if (event_port_outputs.empty()) {
          throw std::runtime_error("edge-sensitive UDP '" + primitive +
        ...
        auto append_row_events = [&](size_t required_port) {
          for (const auto &row : udp.table) {
            if (!row.isEdgeSensitive || row.output == '-' ||
                row.output == 'x') {
              continue;
            }
            const auto inspected =
        ...
        const bool primary_has_pos =
            std::ranges::find(event_keys, std::pair{*primary_event_port,
                                                    OPTO_SLANG_EDGE_POS}) !=
            event_keys.end();
        const bool primary_has_neg =
            std::ranges::find(event_keys, std::pair{*primary_event_port,
                                                    OPTO_SLANG_EDGE_NEG}) !=
            event_keys.end();
        ...
        for (const auto &row : udp.table) {
          ...
          if (row.isEdgeSensitive) {
            predicate =
                lower_udp_edge_predicate(design, instance, ports, row,
                                         *output_lvalue, implicit_event_port)
                    .predicate;
          } else {
            const auto control =
                inspect_udp_async_control(instance, row, ports.size() - 1);
            ...
            predicate = lower_udp_level_predicate(design, instance, ports, row,
                                                  output_lvalue);
          }
          if (!predicate.reachable) {
            continue;
          }
          updates.push_back(GuardedEffectData{
              predicate.expression,
              {
        ...
        module.procedures.push_back(make_guarded_procedure(
            std::move(updates), OPTO_SLANG_PROCEDURE_FLOP, std::move(events),
            source_span(design, output)));
        return;
      }
      std::vector<GuardedEffectData> updates;
      ...
      for (const auto &row : udp.table) {
        ...
        auto predicate = lower_udp_level_predicate(design, instance, ports, row,
                                                   output_lvalue);
        if (!predicate.reachable) {
          continue;
        }
      ...
      if (updates.empty()) {
        throw std::runtime_error("sequential UDP '" + primitive +
                                 "' has no binary-reachable update row");
      }
      module.procedures.push_back(make_guarded_procedure(
          std::move(updates), OPTO_SLANG_PROCEDURE_COMB_OR_LATCH, {},
          source_span(design, output)));
      return;
    }
    ...
    for (auto row = udp.table.rbegin(); row != udp.table.rend(); ++row) {
      ...
      if (row->output != '0' && row->output != '1') {
        throw std::runtime_error("combinational UDP '" + primitive +
                                 "' has unsupported output symbol '" +
                                 std::string(1, row->output) + "'");
      }

      auto predicate =
          lower_udp_level_predicate(design, instance, ports, *row, nullptr);
      if (!predicate.reachable) {
        continue;
      }
      auto *row_value = make_udp_logic_value(design, row->output, output);
      result = predicate.expression
                   ? make_mux_expr(design, predicate.expression, row_value,
                                   result, output)
                   : row_value;
    }
  ...
  if (primitive == "buf" || primitive == "not") {
    require_count(2);
    const auto &input =
        require_primitive_port(instance, ports, ports.size() - 1);
    auto *value = lower_expr(design, input);
    for (size_t output_index = 0; output_index + 1 < ports.size();
         ++output_index) {
      auto *result = value;
      if (primitive == "not") {
        result =
            make_unary_expr(design, OPTO_SLANG_UNARY_BIT_NOT, value, input);
      }
  ...
  if (primitive == "pullup" || primitive == "pulldown") {
    require_count(1, 1);
    const auto &output = require_primitive_port(instance, ports, 0);
    append_assign(0, make_unsigned_constant_expr(
                         design, primitive == "pullup" ? 1 : 0, 1, output));
    return;
  }

  if (primitive == "and" || primitive == "or" || primitive == "xor" ||
      primitive == "nand" || primitive == "nor" || primitive == "xnor") {
    require_count(3);
    ...
    auto *result =
        lower_expr(design, require_primitive_port(instance, ports, 1));
    for (size_t input_index = 2; input_index < ports.size(); ++input_index) {
      const auto &input = require_primitive_port(instance, ports, input_index);
      result = make_binary_expr(design, op, result, lower_expr(design, input),
                                input);
    }
    if (primitive == "nand" || primitive == "nor" || primitive == "xnor") {
      result = make_unary_expr(design, OPTO_SLANG_UNARY_BIT_NOT, result,
  ...
  if (primitive == "bufif0" || primitive == "bufif1" || primitive == "notif0" ||
    ...
    auto *result = active_high ? make_mux_expr(design, control, data,
                                               high_impedance, control_expr)
                               : make_mux_expr(design, control, high_impedance,
                                               data, control_expr);
    append_assign(0, result);
    return;
  }
...
void validate_net_semantics(const NetSymbol &net) {
  ...
  if (net.getDelay()) {
    throw std::runtime_error("delay on net '" + name +
                             "' is not supported for synthesis");
  }
  const auto [drive0, drive1] = net.getDriveStrength();
  if (drive0 || drive1) {
    throw std::runtime_error("drive strength on net '" + name +
                             "' is not supported for synthesis");
  }
...
void validate_continuous_assign_semantics(
    const ContinuousAssignSymbol &assign) {
  if (assign.getDelay()) {
    throw std::runtime_error(
  ...
  if (drive0 || drive1) {
    throw std::runtime_error("drive strength on continuous assignment is not "
...
std::string specialization_signature(const InstanceBodySymbol &body) {
  ...
  for (auto *parameter : body.getParameters()) {
    ...
    if (parameter->symbol.kind == SymbolKind::Parameter) {
      ...
      signature += printed_type(
          parameter->symbol.as<TypeParameterSymbol>().targetType.getType());
    } else {
      throw std::runtime_error(
          "elaborated module contains an unsupported parameter kind");
    }
  }
  for (auto *port_symbol : body.getPortList()) {
    ...
    signature +=
        "|port:" + copy_string(port.name) + "=" + printed_type(port.getType());
  }
  return signature;
}
...
std::string specialization_name(std::string_view definition,
                                std::string_view signature) {
  std::ostringstream stream;
  stream << definition << "__P" << std::hex << std::setw(16)
         << std::setfill('0') << stable_signature_hash(signature);
  return stream.str();
}
...
std::string module_name_for_body(const Context &design,
                                 const InstanceBodySymbol &body) {
  auto found = design.body_names.find(&body);
  if (found == design.body_names.end()) {
    throw std::runtime_error(
        "elaborated module body has no registered specialization name");
  }
  return found->second;
}
...
void lower_body(
    ModuleLoweringContext &design, const ModuleMembers &members,
  ...
  for (auto *port_symbol : body.getPortList()) {
    ...
    if (port_symbol->kind == SymbolKind::InterfacePort) {
      ...
      if (leaves.empty()) {
        throw std::runtime_error("interface port '" + copy_string(port.name) +
      ...
      for (size_t element = 0; element < leaves.size(); ++element) {
        for (auto signal : interface_signals(*leaves[element], modport_name)) {
          ...
          auto name = flattened_interface_port_name(port.name, element,
                                                    leaves.size(), signal.name);
          if (!known_value_names.insert(name).second) {
            throw std::runtime_error("duplicate flattened interface port '" +
                                     name + "'");
          }
          if (signal.value) {
    ...
    if (port_symbol->kind == SymbolKind::MultiPort) {
      ...
      if (name.empty()) {
        reject_external_port_projection(
            port.location, "unnamed concatenated external ports cannot be "
                           "represented in the Opto module interface");
      }
      if (!known_value_names.insert(name).second) {
        reject_external_port_projection(
      ...
      if (port.ports.empty()) {
        reject_external_port_projection(port.location,
                                        "external port '" + name +
      ...
      for (auto *component : port.ports) {
        ...
        projection.parts.push_back(ExternalPortPart{
            component,
            checked_width(lowered_type_width(component->getType()),
                          component->name),
        });
      }
    ...
    if (port_symbol->kind != SymbolKind::Port) {
      throw std::runtime_error(
          "unsupported " + copy_string(toString(port_symbol->kind)) +
          " port in module '" + copy_string(definition.name) + "'");
    }
    const auto &port = port_symbol->as<PortSymbol>();
    auto name = copy_string(port.name);
    if (!known_value_names.insert(name).second) {
    ...
    if (internal_expression) {
      ...
      external_port_projections.push_back(ExternalPortProjection{
          name,
          port.direction,
          checked_width(lowered_type_width(port.getType()), port.name),
      ...
      if (port.internalSymbol->kind == SymbolKind::Net) {
        ...
        design.value_names.emplace(&port.internalSymbol->as<VariableSymbol>(),
                                   name);
      }
    }
    OptoSlangPortData lowered{
  ...
  for (auto *var : members.variables) {
    if (!is_procedural_local(body, *var) && var->getInitializer()) {
      throw std::runtime_error("module variable initializer for '" +
                               copy_string(var->name) +
                               "' is not supported for synthesis");
    }
  ...
  for (auto *interface_instance : members.interface_instances) {
    for (const auto &signal : interface_signals(*interface_instance, {})) {
      ...
      if (!inserted && found->second != name) {
        throw std::runtime_error(
            "interface signal maps to conflicting module nets");
      }
      add_value_as_net(design, module, known_value_names, *signal.value,
                       std::move(name), false);
    }
  }
  for (auto *net : members.nets) {
    ...
    auto name = unique_internal_name(known_value_names,
                                     procedural_local_base_name(body, *net));
  ...
  for (auto *var : members.variables) {
    ...
    auto name = unique_internal_name(known_value_names,
                                     procedural_local_base_name(body, *var));
  ...
  for (const auto &port : module.ports) {
    design.value_shapes.emplace(port.name,
                                ValueShape{port.width, port.is_signed});
  }
  ...
  for (auto *interface_instance : members.interface_instances) {
    for (auto *connection : interface_instance->getPortConnections()) {
      ...
      if (!port.internalSymbol ||
          !ValueSymbol::isKind(port.internalSymbol->kind)) {
        throw std::runtime_error("interface port '" + copy_string(port.name) +
      ...
      case ArgumentDirection::Out:
        module.assigns.push_back(OptoSlangAssignData{
            lower_signal_expr(design,
                              instance_connection_expression(*connection)),
            internal_value,
        });
        break;
      ...
      case ArgumentDirection::Ref:
        throw LoweringFailure(
            OPTO_SLANG_LOWERING_UNSUPPORTED_PROFILE, 5, port.location,
  ...
  for (auto *child : members.instances) {
    ...
    for (auto *connection : child->getPortConnections()) {
      ...
      if (connection->port.kind == SymbolKind::InterfacePort) {
        ...
        for (size_t element = 0; element < leaves.size(); ++element) {
          for (auto signal :
               interface_signals(*leaves[element], modport_name)) {
            if (modport_name.empty()) {
              signal.direction =
            ...
            instance.connections.push_back(OptoSlangConnectionData{
                flattened_interface_port_name(port.name, element, leaves.size(),
                                              signal.name),
                signal.connection
  ...
  for (auto *child : members.unresolved_instances) {
    ...
    if (!syntax || syntax->kind != SyntaxKind::HierarchicalInstance) {
      throw std::runtime_error("unresolved instance '" + instance.name +
                               "' has no instance syntax");
    }
    ...
    for (auto *port : syntax->as<HierarchicalInstanceSyntax>().connections) {
      ...
      if (port->kind == SyntaxKind::OrderedPortConnection) {
        throw std::runtime_error(
            ...
            instance.name + "' are not supported");
      }
      if (port->kind != SyntaxKind::NamedPortConnection) {
        throw std::runtime_error(
            "unsupported port connection syntax on unresolved instance '" +
            instance.name + "'");
      }
      ...
      if (port_name.empty()) {
        throw std::runtime_error("empty port name on unresolved instance '" +
                                 instance.name + "'");
      }

      if (unresolved_connection_index >= bound_unresolved_connections.size()) {
        throw std::runtime_error("unresolved instance connection was not bound "
                                 "before parallel lowering");
      }
      const Expression *expr =
          bound_unresolved_connections[unresolved_connection_index++];
  ...
  if (unresolved_connection_index != bound_unresolved_connections.size()) {
    throw std::runtime_error("unresolved instance connection binding count "
  ...
  for (auto *net : members.nets) {
    ...
    lhs.signal_name =
        intern_string(design, registered_value_name(design, *net));
    module.assigns.push_back(OptoSlangAssignData{
...
void collect_module_jobs(OptoSlangSnapshot &design,
                         const InstanceBodySymbol &body,
...
std::unique_ptr<OptoSlangModulePayload>
lower_module_job(const OptoSlangSnapshot &shared,
                 const ModuleLoweringJob &job) {
  auto module = std::make_unique<OptoSlangModulePayload>();
  ModuleLoweringContext lowering(*module, *job.body, shared.body_names,
                                 shared.source_manager);
...
struct OptoSlangCompilationState {
  ...
  OptoSlangCompilationState(std::unique_ptr<Driver> driver,
                            std::unique_ptr<Compilation> compilation,
                            std::vector<ModuleLoweringJob> jobs)
      : driver(std::move(driver)), compilation(std::move(compilation)),
        jobs(std::move(jobs)) {}
};
...
void opto_slang_prepare_module_names(
  ...
  for (auto *body : bodies) {
    ...
    auto found = std::ranges::find_if(
        specializations, [body](const Specialization &specialization) {
  ...
  for (auto &[definition, specializations] : definitions) {
    for (auto &specialization : specializations) {
      ...
      auto [found, inserted] =
          signatures_by_name.emplace(name, specialization.signature);
      if (!inserted && found->second != specialization.signature) {
        throw std::runtime_error(
            "parameter specialization name hash collision for module '" +
            definition + "'");
      }
...
void opto_slang_collect_modules(OptoSlangSnapshot &design,
  ...
  std::ranges::sort(jobs, [&](const auto &left, const auto &right) {
    ...
    return module_name_for_body(design, *left.body) <
           module_name_for_body(design, *right.body);
  });
...
void set_lowering_failure_source(OptoSlangLoweringFailure &failure,
...
OptoSlangStatus opto_slang_materialize_module(OptoSlangSnapshot &design,
                                              size_t module_index) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        set_lowering_failure_source(*target.materialize_failure, design,
                                    design.compilation_state->jobs[module_index]
                                        .body->getDefinition()
                                        .location);
      } catch (...) {
        target.materialize_failure.reset();
