...
template <typename Source, typename View>
bool valid_view(const Source *source, const View *view) {
  return source && view;
}
...
const OptoSlangModulePayload *module_payload(const OptoSlangSnapshot *design,
                                             size_t module_index) {
...
void assign_attribute_view(const OptoSlangAttributeData &attribute,
...
void opto_slang_snapshot_free(OptoSlangSnapshot *design) { delete design; }
...
OptoSlangStatus opto_slang_module_view(const OptoSlangSnapshot *design,
  ...
  try {
    ...
    *view = OptoSlangModuleView{
        payload.attributes.size(), payload.ports.size(),
        payload.nets.size(),       payload.instances.size(),
        payload.assigns.size(),    payload.procedures.size(),
    };
    return OPTO_SLANG_OK;
  } catch (...) {
    return OPTO_SLANG_ERROR;
...
OptoSlangStatus opto_slang_block_view(const OptoSlangProcedureData *procedure,
  ...
  *view = OptoSlangBlockView{block.effects.size(), block.terminator.kind,
                             block.source};
  return OPTO_SLANG_OK;
}
...
opto_slang_switch_arm_view(const OptoSlangProcedureData *procedure,
  ...
  const auto &terminator = procedure->blocks[block_index].terminator;
  if (terminator.kind != OPTO_SLANG_TERMINATOR_SWITCH ||
      arm_index >= terminator.arms.size()) {
    return OPTO_SLANG_ERROR;
  }
