...
/// Gives otherwise-undefined output bits an explicit care-free SSA value.
///
/// This is a source-semantic completion step, not a driver repair. It is
/// deliberately limited to top-level output ports after all source drivers
/// have been lowered. Missing internal and generated drivers remain invalid.
pub(super) fn seal_observable_dont_cares(
    ...
    for (signal, ty, source) in outputs {
        ...
        while bit < row.len() {
            ...
            let lsb = u32::try_from(start)
                .map_err(|_| crate::SynthError::capacity("output bit index exceeds u32"))?;
            let width = u32::try_from(bit - start)
                .map_err(|_| crate::SynthError::capacity("output width exceeds u32"))?;
            ...
            let target = if lsb == 0 && width == ty.width() {
                ...
                word::LValue::signal(signal).with_range(word::BitRange {
                    msb: lsb
                        .checked_add(width - 1)
                        .ok_or_else(|| crate::SynthError::capacity("output range exceeds u32"))?,
                    lsb,
                })
            };
            module
