...
pub(crate) struct WordMappedSignals {
    ...
    suppressed_aliases: BTreeSet<BoundaryAlias>,
}
...
impl WordMappedSignals {
    pub(crate) fn from_observations_with_aliases(
        ...
        suppressed_aliases: &[BoundaryAlias],
        ...
        Ok(Self {
            signals,
            boundary_aliases,
            suppressed_aliases: suppressed_aliases.iter().copied().collect(),
        })
    }
    ...
    pub(crate) fn validate_alias(
        ...
        let normalize = |source| {
            if let Some(MappedValueSignal::Constant(value)) = self.signals.get(&target) {
                Some(BoundaryAliasSource::Constant(*value))
            } else if let Some(BoundaryAliasSource::Value(value)) = source
                && let Some(MappedValueSignal::Constant(value)) = self.signals.get(&value)
            {
                Some(BoundaryAliasSource::Constant(*value))
            } else if let Some(BoundaryAliasSource::Value(value)) = source
                && self.signals.get(&target) == self.signals.get(&value)
            {
                None
            } else {
                source
        ...
        let expected = normalize(self.boundary_aliases.get(&target).copied());
        let source = normalize(source);
        if expected != source
            && reconstructed.is_some_and(|source| {
                self.suppressed_aliases
                    .contains(&BoundaryAlias { target, source })
            })
        {
            return Ok(());
        }
        (expected == source).then_some(()).ok_or_else(|| {
            crate::SynthError::invariant(format!(
                "regional boundary alias for {target:?} differs from the frozen substrate: expected {expected:?}, reconstructed {source:?}"
            ))
        })
    }
}
...
type BoundaryAliasPartition = (Box<[BoundaryAlias]>, Box<[BoundaryAlias]>);
...
pub(crate) fn regional_boundary_aliases(
...
) -> Result<BoundaryAliasPartition, crate::SynthError> {
    ...
    for (plan, binding) in plans.iter().zip(bindings) {
        ...
        let cover = crate::mapping::cover::decode_portable_cover(payload)?;
        let outputs = binding.resolve_output_groups(ownership)?;
        for (values, source) in outputs.iter().zip(cover.outputs()) {
            if matches!(
                source,
                LibraryCoverSource::Cell(_) | LibraryCoverSource::CellSecond(_)
            ) && let Some(&canonical) = values.first()
            {
                materialized.insert(canonical);
            }
        }
    }
    aliases.sort_unstable();
    aliases.dedup();
    let aliases = deduplicate_equivalent_aliases(module, &aliases)?;
    let (suppressed, physical): (Vec<_>, Vec<_>) = aliases
        .into_iter()
        .partition(|alias| materialized.contains(&alias.target));
    Ok((physical.into_boxed_slice(), suppressed.into_boxed_slice()))
}
