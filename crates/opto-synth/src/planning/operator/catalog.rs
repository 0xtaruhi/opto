...
impl OperatorCatalog {
    pub(super) fn regional_shell(operation_count: usize) -> Self {
        Self {
            ...
            operator_by_source_operation: vec![MISSING_OPERATOR_ID; operation_count]
                .into_boxed_slice(),
            elided_operations: vec![false; operation_count].into_boxed_slice(),
    ...
    pub(super) fn for_module(
            ...
            let Some(operator) = operator else {
                if let Some(region) = region.as_ref() {
                    for &source_index in &region.operations {
                        elided_operations[source_index] = true;
                    }
                } else if requires_implementation(&module.operations()[index].kind) {
        ...
        Ok(Self {
            ...
            elided_operations: elided_operations.into_boxed_slice(),
            candidates,
            source_operations,
            arithmetic_terms,
        })
    ...
    pub(super) fn is_operation_elided(&self, operation: word::OpId) -> bool {
        self.elided_operations
            .get(operation.index())
            .copied()
            .unwrap_or(false)
    }
...
fn requires_implementation(kind: &word::OpKind) -> bool {
    matches!(
        kind,
        word::OpKind::Binary {
            op: word::BinaryOp::Add
                | word::BinaryOp::Sub
                | word::BinaryOp::Mul
                | word::BinaryOp::Div
                | word::BinaryOp::Mod,
