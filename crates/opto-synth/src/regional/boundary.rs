...
/// Monotone known-value fact propagated through a region boundary.
pub enum BoundaryKnownFact {
    /// No information has reached this port yet.
    Unknown,
    /// Every bit is known, including values wider than 128 bits.
    Constant(opto_ir::ConstBits),
    /// Partial known-bit information for a value no wider than 128 bits.
    Packed(opto_ir::word::KnownBits128),
    /// Incoming paths have no common provable fact.
    Conflict,
}

impl BoundaryKnownFact {
    /// Meet two incoming facts in the finite boundary lattice.
    #[must_use]
    pub fn meet(&self, other: &Self) -> Self {
        use BoundaryKnownFact::{Conflict, Constant, Packed, Unknown};
        match (self, other) {
            (Unknown, fact) | (fact, Unknown) => fact.clone(),
            ...
            (Packed(left), Packed(right)) => meet_packed(*left, *right),
            (Constant(constant), Packed(packed)) | (Packed(packed), Constant(constant)) => {
                meet_constant_packed(constant, *packed)
            }
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
/// Dataflow information and backward liveness for one boundary port.
pub struct BoundaryDataflowContract {
    /// Known-value fact descending monotonically under [`BoundaryKnownFact::meet`].
    pub known: BoundaryKnownFact,
    /// Whether an observable root reaches this boundary in the backward live closure.
    pub live: bool,
}

impl Default for BoundaryDataflowContract {
    fn default() -> Self {
        Self {
            known: BoundaryKnownFact::Unknown,
            live: false,
        }
    }
}

impl BoundaryDataflowContract {
    pub(crate) fn refine(&mut self, incoming: &Self) -> bool {
        let known = self.known.meet(&incoming.known);
        let live = self.live || incoming.live;
        let changed = known != self.known || live != self.live;
        self.known = known;
        self.live = live;
        changed
    }
...
fn meet_packed(
    left: opto_ir::word::KnownBits128,
    right: opto_ir::word::KnownBits128,
) -> BoundaryKnownFact {
    if left.width() != right.width() {
        return BoundaryKnownFact::Conflict;
    }
    let bits = (0..left.width()).map(|index| {
        let left = left.bit(index);
        if left == right.bit(index) && left != opto_ir::word::KnownBit::Unknown {
            left
        } else {
            opto_ir::word::KnownBit::Unknown
        }
    });
    packed_or_conflict(bits)
}

fn meet_constant_packed(
    constant: &opto_ir::ConstBits,
    packed: opto_ir::word::KnownBits128,
) -> BoundaryKnownFact {
    if constant.width() < packed.width() {
        return BoundaryKnownFact::Conflict;
    }
    let bits = (0..packed.width()).map(|index| {
        let known = packed.bit(index);
        let constant_index = constant.width() - 1 - index;
        let constant = constant.as_slice()[constant_index as usize];
        let agrees = matches!(
            (known, constant),
            (opto_ir::word::KnownBit::Zero, opto_ir::BitVal::Zero)
                | (opto_ir::word::KnownBit::One, opto_ir::BitVal::One)
        );
        if agrees {
            known
        } else {
...
fn packed_or_conflict(
    bits: impl IntoIterator<Item = opto_ir::word::KnownBit>,
) -> BoundaryKnownFact {
    let bits = bits.into_iter().collect::<Vec<_>>();
    if bits
        .iter()
        .all(|bit| *bit == opto_ir::word::KnownBit::Unknown)
    {
        return BoundaryKnownFact::Conflict;
    }
    BoundaryKnownFact::Packed(
        opto_ir::word::KnownBits128::from_bits(bits)
            .expect("boundary packed facts have a checked nonzero width"),
    )
}
...
mod tests {
    use super::*;
    use opto_ir::word::{KnownBit, KnownBits128};

    fn packed(bits: &[KnownBit]) -> BoundaryKnownFact {
        BoundaryKnownFact::Packed(KnownBits128::from_bits(bits.iter().copied()).unwrap())
    }

    #[test]
    fn boundary_fact_meet_is_monotone_and_conflict_is_absorbing() {
        let known = packed(&[KnownBit::One, KnownBit::Zero, KnownBit::Unknown]);
        assert_eq!(BoundaryKnownFact::Unknown.meet(&known), known);
        assert_eq!(
            BoundaryKnownFact::Conflict.meet(&known),
            BoundaryKnownFact::Conflict
        );
        assert_eq!(
            known.meet(&BoundaryKnownFact::Conflict),
            BoundaryKnownFact::Conflict
        );
    }

    #[test]
    fn packed_meet_keeps_only_proven_agreement() {
        let left = packed(&[KnownBit::One, KnownBit::Zero, KnownBit::One]);
        let right = packed(&[KnownBit::One, KnownBit::One, KnownBit::Unknown]);
        let BoundaryKnownFact::Packed(meet) = left.meet(&right) else {
            panic!("packed facts retain their agreeing low bit");
        };
        assert_eq!(meet.bit(0), KnownBit::One);
        assert_eq!(meet.bit(1), KnownBit::Unknown);
        assert_eq!(meet.bit(2), KnownBit::Unknown);
    }

    #[test]
    fn incompatible_constants_meet_at_conflict() {
        let zero = BoundaryKnownFact::Constant(opto_ir::ConstBits::from_bin_str("0").unwrap());
        let one = BoundaryKnownFact::Constant(opto_ir::ConstBits::from_bin_str("1").unwrap());
        assert_eq!(zero.meet(&one), BoundaryKnownFact::Conflict);
    }
}
