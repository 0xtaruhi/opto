...
impl TimingContext {
    ...
    #[allow(
        ...
        reason = "this is the canonical SDC ordering table; keeping all command families in one \
                  traversal makes serialization order and omission rules directly auditable"
    )]
    pub fn write_sdc(
        &self,
        ...
        let mut occupied_clock_sources = BTreeSet::new();
        for clock in &self.clocks {
            let needs_add = clock
                .sources
                .iter()
                .any(|source| occupied_clock_sources.contains(source));
            ...
            occupied_clock_sources.extend(clock.sources.iter().copied());
        }

        for (&port, slots) in &self.input_transitions {
            let object = object_expression(&[port.erase()])?;
            write_edge_delay_slots(&mut output, "set_input_transition", slots, &object);
        }
        for (&port, slots) in &self.loads {
            let object = object_expression(&[port.erase()])?;
            write_edge_delay_slots(&mut output, "set_load", slots, &object);
        }
        for (&endpoint, slots) in &self.resistances {
            let object = object_expression(&[endpoint.object_id()])?;
            match endpoint {
                TimingEndpoint::Port(_) => {
                    write_edge_delay_slots(&mut output, "set_drive", slots, &object);
                }
        ...
        for clock in &self.clocks {
            ...
            if clock.propagated {
                push_sdc_line(&mut output, &format!("set_propagated_clock {object}"));
            }
        }

        for (&key, &value) in &self.clock_uncertainties {
        ...
        for (&endpoint, &value) in &self.case_analysis {
            push_sdc_line(
                &mut output,
                &format!(
                    "set_case_analysis {} {}",
        ...
        for kind in [
            ...
            for (path_index, path_name) in [(0, "clock"), (1, "data")] {
                for (early_late, early_late_name) in [(0, "early"), (1, "late")] {
                    for edge in TimingEdge::ALL {
                        let value = self.timing_derates.0[kind.index()][path_index][early_late]
                            [edge.index()];
        ...
        let mut written_clock_groups = BTreeSet::new();
        for exception in &self.path_exceptions {
            let Some((kind, name, comment)) = parse_clock_group_marker(&exception.comment) else {
                continue;
            };
            let mut from = exception
                .from
                .objects()
                .iter()
            ...
            if !written_clock_groups.insert((
                kind.to_string(),
                name.to_string(),
                from.clone(),
                to.clone(),
        ...
        {
            write_path_exception(&mut output, exception, &mut object_expression)?;
        }
        for (kind, command) in [
            ...
            for rule in self.design_rule_constraints(kind).iter() {
                ...
                match rule.scope {
                    ...
                    DesignRuleScope::ClockAndData => {
                        line.push_str(" -clock_path -data_path");
                    }
                    DesignRuleScope::All => {}
                }
