...
fn residual_cycle(graph: &TimingGraph, indegree: &[usize], start: usize) -> String {
    let mut predecessor = std::collections::HashMap::<usize, (usize, GraphArcId)>::new();
    for (from, arcs) in graph.outgoing.iter().enumerate() {
        ...
        for &arc in arcs {
            let to = graph.arc(arc).to.index();
            if indegree[to] != 0 {
                predecessor.entry(to).or_insert((from, arc));
            }
        }
    }
    ...
    loop {
        if let Some(&first) = seen.get(&net) {
            let mut text = walk[first..]
                ...
                .rev()
                .map(|&net: &usize| {
                    let instance = predecessor
                        .get(&net)
                        .map(|(_, arc)| graph.arc(*arc).instance.raw());
                    instance.map_or_else(
                        || graph.net_names[net].to_string(),
                        |instance| format!("{} -[cell {instance}]->", &graph.net_names[net]),
                    )
                })
                .collect::<Vec<_>>();
        ...
        walk.push(net);
        let Some(&(previous, _)) = predecessor.get(&net) else {
