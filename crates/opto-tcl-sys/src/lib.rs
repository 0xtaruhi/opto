...
pub enum TclError {
    ...
    RegisterFilesystem,
    /// Registration of process-exit cleanup failed.
    #[error("failed to register embedded Tcl process cleanup")]
...
thread_local! {
    static LIVE_THREAD_INTERPRETERS: Cell<usize> = const { Cell::new(0) };
}
...
fn retain_thread_runtime() {
    LIVE_THREAD_INTERPRETERS.with(|count| {
        count.set(
            count
                .get()
                .checked_add(1)
                .expect("live Tcl interpreter count overflowed"),
        );
    });
}

fn release_thread_runtime() {
    LIVE_THREAD_INTERPRETERS.with(|count| {
        let remaining = count
            .get()
            .checked_sub(1)
            .expect("a Tcl interpreter was released on the wrong thread");
        count.set(remaining);
        if remaining == 0 {
            // SAFETY: the final interpreter owned by this thread has just been destroyed, and
            // `Interpreter` is neither `Send` nor `Sync`, so no interpreter on another thread can
            // refer to this thread's Tcl state.
            unsafe { ffi::Tcl_FinalizeThread() };
        }
    });
}
...
fn initialize_process() -> Result<(), TclError> {
    INITIALIZATION
        .get_or_init(|| {
            ...
            // SAFETY: the callback has C ABI, never unwinds, and remains valid for the lifetime of
            // the process. `OnceLock` registers it at most once after Tcl initializes successfully.
            if unsafe { atexit(finalize_tcl_process) } != 0 {
                return Err(TclError::RegisterProcessCleanup);
            }
            Ok(())
        })
        .clone()
...
extern "C" fn finalize_tcl_process() {
    // SAFETY: the C runtime invokes this only during normal process shutdown, after Rust's main
    // function and all stack-owned interpreters have completed. Tcl documents repeated
    // finalization as safe.
    unsafe { ffi::Tcl_Finalize() };
}
...
mod tests {
    ...
    #[test]
    fn finalizes_thread_state_after_the_last_interpreter() {
        let first = Interpreter::new().unwrap();
        let second = Interpreter::new().unwrap();
        drop(first);

        // SAFETY: `second` is still live after the other interpreter on this thread is dropped.
        let code = unsafe { ffi::Tcl_Eval(second.as_ptr(), c"expr {20 + 22}".as_ptr()) };
        assert_eq!(code, TCL_OK, "{}", second.result_string());
        assert_eq!(second.result_string(), "42");
        drop(second);

        let recreated = Interpreter::new().unwrap();
        // SAFETY: Tcl supports initialization after `Tcl_FinalizeThread`, and `recreated` is live.
        let code = unsafe { ffi::Tcl_Eval(recreated.as_ptr(), c"list restarted".as_ptr()) };
        assert_eq!(code, TCL_OK, "{}", recreated.result_string());
        assert_eq!(recreated.result_string(), "restarted");
    }
