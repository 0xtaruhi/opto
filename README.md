...
supported. The nightly frontend and formal qualification workflow verifies
the exact required set and also checks that every capability gap, exclusion,
and deferred proof still matches its reviewed, hashed audit entry. It remains
outside required pull-request CI because of its external tools and runtime.
